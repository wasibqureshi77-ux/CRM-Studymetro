...
export default function DashboardPage() {
  ...
  if (user?.role === 'COUNSELLOR') {
    ...
    const todayNewLeadsCount = leads.filter(l => new Date(l.createdAt) >= today && new Date(l.createdAt) <= todayEnd).length;
    
    const todayFollowups = followups.filter(f => {
      const fDate = new Date(f.followupDate);
      return f.status === 'SCHEDULED' && fDate >= today && fDate <= todayEnd;
    });

    const pendingFollowups = followups.filter(f => f.status === 'SCHEDULED');
    const overdueFollowups = followups.filter(f => f.status === 'SCHEDULED' && new Date(f.followupDate) < new Date());

    const upcomingAppointments = followups
      .filter(f => f.status === 'SCHEDULED' && new Date(f.followupDate) >= new Date())
      .sort((a, b) => new Date(a.followupDate).getTime() - new Date(b.followupDate).getTime());

    const filteredUpcoming = upcomingAppointments.filter(f => {
      const leadName = f.lead ? `${f.lead.firstName} ${f.lead.lastName || ''}`.toLowerCase() : '';
      return leadName.includes(searchQuery.toLowerCase());
    });
    ...
    return (
      <div style={{ paddingBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border-color)', background: '#fff', gap: '16px', flexWrap: 'wrap' }}>
          <div>
            <h2 style={{ fontSize: '15px', fontWeight: 700 }}>Counsellor Dashboard</h2>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Welcome back, {user?.firstName}. Managing your assigned students.</p>
          </div>
        ...
        <section className="dashboard-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
          <div className="kpi-card" style={{ borderLeft: '3px solid #3b82f6' }}>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>Assigned leads</div>
          </div>

          <div className="kpi-card" style={{ borderLeft: '3px solid #22c55e' }}>
            <div className="kpi-label">Today's New Leads</div>
            <div className="kpi-value">{todayNewLeadsCount}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>Registered today</div>
          </div>

          <div className="kpi-card" style={{ borderLeft: '3px solid #8b5cf6' }}>
            <div className="kpi-label">Today's Followups</div>
            <div className="kpi-value">{todayFollowups.length}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>Due today</div>
          </div>

          <div className="kpi-card" style={{ borderLeft: '3px solid #f59e0b' }}>
            <div className="kpi-label">Pending Followups</div>
            <div className="kpi-value" style={{ color: '#d97706' }}>{pendingFollowups.length}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>Awaiting contact</div>
          </div>

          <div className="kpi-card" style={{ borderLeft: '3px solid #ef4444' }}>
            <div className="kpi-label">Overdue Followups</div>
          ...
          <div style={{ flex: 1, minWidth: '300px', background: '#fff', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '16px' }}>
            ...
            {filteredUpcoming.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {filteredUpcoming.slice(0, 8).map((f) => (
